...
export const getPrestamos = async (req, res) => {
  try {
    const [result] = await pool.query('SELECT p.idPrestamo, date_format(p.fecha , "%Y-%m-%d") AS fecha,TIME_FORMAT( p.horaPrestamo, "%h:%i %p") as horaPrestamo, TIME_FORMAT( p.horaDevolucion, "%h:%i %p") as horaDevolucion, p.estado, d.nombre as nombreDocente, u.nombre as nombreUsuario from prestamo p INNER JOIN docente d on p.docente = d.idDocente INNER JOIN usuario u on p.usuario = u.idUsuario order by p.idPrestamo'); 
    res.json(result);
  } catch (error) {
